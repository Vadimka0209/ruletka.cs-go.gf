...
if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
{
    ...
    foreach($adminss as $culoarez){ 
        if($culoarez == $steamprofile['steamid']) { 
            $steamprofile['personaname']= '<font color=#33D633><b>'.$steamprofile['personaname'].'</b></font>';
    ...
    $person = '
               ...
               '.$massage.'
                <div class="caht-name"><img class="caht-ava" src="'.$steamprofile['avatarmedium'].'" width="20px"><a href="prredirect.php?id='.$steamprofile['steamid'].'&amp;name='.base64_encode($steamprofile['personaname']).'" target="_blank" title="'.$today.'">'.$steamprofile['personaname'].'</a>:</div>
                <div class="chat-msg">
